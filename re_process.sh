...
${root}/check_schedule.py "${csv}" 2> errors.txt | sed '/\*\*\*/,$d' > campers.txt
${root}/check_schedule.py "${csv}" 2> errors.txt | sed '1,/\*\*\*/d' > activities.txt
...
python ${root}/family2pdf.py "${csv}" .

python ../pad_pdf.py *_timetable.pdf > all_timetables.pdf
}
